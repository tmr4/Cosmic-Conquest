...
: cc-xy 1- swap 1- swap at-xy ;

: array create dup c, * allot does> rot 1- over c@ * + + ;

: does_scan does> swap 2 * + dup c@ dup #15 > if #256 - then swap 1+ c@ dup #15 > if #256 - then ; 

#30 constant size size 2 * constant no-of-stars size 3 * 2 / constant no-of-planets 4 constant no-of-b-holes #200 constant w1 5 constant w2 #10 constant w3 #450 constant comp-start variable temp1 variable vtax variable c-legions variable class-totals variable c-fleets variable len variable troops variable rand1 variable rand2 variable x variable y variable buy-v variable leg variable new variable computer variable diff variable c-planets variable planets variable fleet-flag variable credit size size array galaxy size size array info1 size size array info2 2 6 array fleets #120 constant srl variable scanc #168 constant scanl 
...
: clr-txt #23 #15 do 1 i cc-xy #30 spaces loop ;
...
: show-cmd txt-home ask? show-dir ." F - Fire" cr ." T - Tax" cr ." O - Switch Fleets" cr ." L - Land" cr ." X - Exit" ;
...
: sr-scan case 2 of '*' endof 4 of 'O' endof 5 of '0' endof #132 of '@' endof #16 of 'P' endof #17 of 'E' endof $20 swap endcase ;

: lr-scan if '.' else $20 then ;

: scan scanc @ sxy 2dup #6 + swap 2 * #45 + swap cc-xy 1 f c@ + 5 - edge-check swap 2 f c@ + 5 - edge-check galaxy c@ scanc @ srl < if sr-scan else lr-scan then emit scanc @ 1+ dup scanl > if drop 0 then scanc ! ;
...
: up-stats #15 1 cc-xy score 6 .r #16 4 cc-xy c-fleets @ 5 .r #16 5 cc-xy c-planets @ 5 .r #15 8 cc-xy credit @ 6 .r #16 9 cc-xy planets @ 5 .r #10 #11 cc-xy 1 2 fleets c@ 2 .r #13 #11 cc-xy 1 1 fleets c@ 2 .r #16 #11 cc-xy 2 2 fleets c@ 2 .r #19 #11 cc-xy 2 1 fleets c@ 2 .r #11 #12 cc-xy 1 3 fleets @ 4 .r #17 #12 cc-xy 2 3 fleets @ 4 .r #11 #13 cc-xy 1 5 fleets @ 4 .r #17 #13 cc-xy 2 5 fleets @ 4 .r msg-home ;

: up-scan 0 scanc ! up-stats ;

: up-display clr-msg up-scan show-cmd ;
...
: deploy msg how-many? 5 f @ min temp1 ! 5 f @ temp1 @ - 5 f ! xy@ info2 c@ temp1 @ + #255 min xy@ info2 c! ;

: buy clr-txt buy-v @ 0= if 5 buy-v ! random1 5 / xy@ info1 c@ #10 / + 1+ dup temp1 ! 1 #16 cc-xy ." Cost per ship = " 2 .r how-many? credit @ temp1 @ / min dup 3 f @ + 3 f ! temp1 @ * credit @ swap - credit ! #16 1 f c@ 2 f c@ galaxy c! else msg ." No ships available" then ;

: friendly-resp key?-loop msg key '0' - case 1 of deploy 0 endof 2 of gather 0 endof 3 of buy 0 endof 4 of enlist 0 endof clr-txt 1 swap endcase ;

: friendly-planet begin clr-txt 1 #15 cc-xy ." Class " xy@ info1 c@ 8 / 2 .r ."  planet" cr ." Local garrison is " xy@ info2 c@ 3 .r ."  legions" cr ask? ." 1 - Leave legions on planet" cr ." 2 - Gather legions from planet" cr ." 3 - Buy ships" cr ." 4 - Enlist troops" cr ." Any other key to leave planet" friendly-resp until up-display ;
...
: tax 0 vtax ! msg ." Tax collected = 0" size 1+ 1 do size 1+ 1 do i j galaxy c@ #132 = if i j info1 c@ 3 * 5 / vtax @ + dup vtax ! #17 #23 cc-xy 5 .r i j info1 c@ 7 and if i j info1 dup c@ 1- swap c! else i x ! j y ! revolt? then then loop loop credit @ vtax @ + credit ! up-display ;
